...
def calculate_ES(scenario_model, agg_level):
    ...
    for group in grouping:    # this for loop could potentially be parallelized
        ...
        for date in date_range_str: # iterate across dates
            ...
            prepped_reals_all = prep_reals_for_scoring(reals_all, num_assets=3, num_days=1, num_hours=24, full_day=True) # three asset because its all of them
            prepped_sims_all = prep_sims_for_scoring(sims_all, num_scenarios=1000, num_assets=3, num_days=1, num_hours=24,
                                                 full_day=True)
        ...
        output.to_csv("/projects/PERFORM/Analytics_Platform/Outputs/ES_" + scenario_model.name + "_" + group + ".csv")
    return None
